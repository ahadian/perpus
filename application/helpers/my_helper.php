...
function getJenis($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT nama FROM jenis WHERE id = '$id'")->row();
	$nama		= empty($q_ambil) ? "-" : $q_ambil->nama;
	return $nama;
}
function getLokasi($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT nama FROM lokasi WHERE id = '$id'")->row();
	$nama		= empty($q_ambil) ? "-" : $q_ambil->nama;
	return $nama;
}
...
function getNama($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT nama FROM anggota WHERE id = '$id'")->row();
	$nama		= empty($q_ambil) ? "-" : $q_ambil->nama;
	return $nama;
}
function getJudul($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT judul FROM buku WHERE id = '$id'")->row();
	return $q_ambil->judul;
}

function getPengarang($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT name FROM authors WHERE id = '$id'")->row();
	return $q_ambil->name;
}

function getPenerbit($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT name FROM publishers WHERE id = '$id'")->row();
	return $q_ambil->name;
}
...
function adddate($hari) {
	...
	$waktu 		= $hari * 2;
	$CI 		=& get_instance();
	$q_ambil	= $CI->db->query("SELECT tanggal FROM libur ORDER BY id ASC")->result();
	$jum_libnas	= 0;
	$jum_hari	= 0;
	$jumlah_minggu	= 0;
